...
passports = """hcl:#6b5442 ecl:brn iyr:2019
...
t = []
temp = []
for i in passports.split("\n"):
    if i=="":
        t.append(temp)
        temp = []
    temp.extend([i])
t.append(temp)
nec = ["byr","iyr","eyr","hgt","hcl","ecl","pid"]
t = [" ".join(x) for x in t]
...
present = [sorted(x.split(" ")[1:]) for x in present ] #All str have an empty space at the beginnning so [1:]
count = 0
dictlist = []
for p in present:
    dic = {}
