...
for i in range(len(text)):
    ct = text[i].count(letter[i])
    if int(nums[i][0])<=int(ct)<=int(nums[i][1]):
        count+=1
print(count)
...
for i in range(len(text)):
    flag = 0
    if text[i][int(nums[i][0])] == letter[i]:
        flag = 1
    if text[i][int(nums[i][1])] == letter[i]:
        if flag ==1:
            flag = 0
        else:
            flag =1
    ...
    if flag ==0:
        print(nums[i][0],nums[i][1],letter[i],text[i])
        ncount+=1
